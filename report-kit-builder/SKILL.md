...
- **差异化回流提示（2026-09-23 按攒批规则修订）**：任何项目用本 skill 做差异化修改并触发自我迭代（skill 本地更新）后，改动**一律攒批在项目本地/分支，不主动提示提交、不推送**。仅两种情况例外：① 用户确认项目完全闭环（亲口说「已经确认闭环了」）后，agent 才提示「是否把更新的技能内容/源码上传给原作者（维护机）合并」，经用户同意后按仓库根 `COLLAB_PROTOCOL.md` 走分支→PR 流程统一回流（脱敏门禁先过，业务专属内容剔除）；② 用户明确点名「现在就修 skill」的紧急缺陷。详见 COLLAB_PROTOCOL 第 7 条回灌时机/攒批规则。
