...
# 项目结构适配：05_app 本身是 git 仓则用之（麻辣结构）；否则取其下唯一 git 仓（宜兴 05_app/yixing-app 结构）
APP="$R/05_app"
if [ ! -d "$APP/.git" ]; then
...
cp -r "$APP/docs" "$K/assets/docs" 2>/dev/null || echo "SKIP: $APP/docs 不存在（项目无独立 docs，保留 skill 现有 docs）"
